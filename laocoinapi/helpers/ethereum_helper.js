...
const createAccountAndSendSignedTransaction = async () => {
  ...
  const signedTransaction = await newAccountCheck.signTransaction({
    to: newAccountCheck.address,
    value: 100000000000000000, gas: 2000000
  }, newAccountCheck.privateKey);
...
exports.getTransactionsByAccount = async (account, startBlockNumber, endBlockNumber) => {
  ...
  for (let i = startBlockNumber; i <= endBlockNumber; i += 1) {
    ...
    console.log(accountBlockTransactions);
...
exports.getBlockTransactionsByAccountAdmin = async (account, startBlockNumber, endBlockNumber) => {
  console.log('===== getTransactionsByAccount() =====');
  const accountTransactions = [];
  ...
  for (let i = startBlockNumber; i <= endBlockNumber; i += 1) {
    ...
    console.log(`Searching block ${i}`);
    // }
    const accountBlockTransactions = await getBlockTransactionsByAccountAdmin(i, account);
    console.log(accountBlockTransactions);
    accountTransactions.push(...accountBlockTransactions);
    console.log(`transaction count: ${accountTransactions.length}`);
  }
...
const getBlockTransactionsByAccountAdmin = async (blockNumber, account) => {
  console.log(`===== getBlockTransactionsByAccount(${blockNumber}, ${account}) =====`);
  const accountBlockTransactions = [];
