...
exports.getUserTransactionsAdmin = async (req, res, next) => {
  const user = await User.findById(req.params.user_id).exec();
  if (!user) return next(createError(404, 'No User'));

  const accountTransactions = await ethereumHelper.getBlockTransactionsByAccountAdmin(user.ethereum_account_address);
...
exports.createUserTransaction = async (req, res, next) => {
  ...
  const userRecipient = await User.findOne({ ethereum_account_address: req.body.recipient }).exec();
